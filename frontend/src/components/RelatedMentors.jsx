...
const RelatedMentors = ({ speciality, docId }) => {
    ...
    useEffect(() => {
        if (mentors.length > 0 && speciality) {
            const mentorsData = mentors.filter((doc) => doc.speciality === speciality && doc._id !== docId)
            setRelDoc(mentorsData)
        }
    }, [mentors, speciality, docId])

    return (
        <div className='flex flex-col items-center gap-4 my-16 text-[#262626]'>
            <h1 className='text-3xl font-medium'>Related Mentors</h1>
            <p className='sm:w-1/3 text-center text-sm'>Simply browse through our extensive list of Skillfull Mentors.</p>
